...
from gto.config import CONFIG, CONFIG_FILE
...
def test_api(init_showcase):
    ...
    _check_dict(nn_version, dict(object='nn',
                                 name='v1',
                                 author=getpass.getuser(),
                                 commit_hexsha=first_commit.hexsha,
                                 unregistered_date=None), {"creation_date"})
    assert len(nn_object.labels) == 1
    nn_label = nn_object.labels[0]
    assert isinstance(nn_label, BaseLabel)
    _check_dict(nn_label, dict(object='nn', version='v1', name='staging',
                               author=getpass.getuser(),
                               commit_hexsha=first_commit.hexsha,
                               unregistered_date=None), {"creation_date"})
    ...
    _check_dict(rf_ver1, dict(object='rf', name='v1',
                              author=getpass.getuser(), commit_hexsha=first_commit.hexsha,
                              unregistered_date=None), {"creation_date"})
    _check_dict(rf_ver2, dict(object='rf', name='v2',
                              author=getpass.getuser(), commit_hexsha=second_commit.hexsha,
                              unregistered_date=None), {"creation_date"})
    ...
    _check_dict(rf_l1, dict(object='rf', version='v1', name='production',
                            author=getpass.getuser(),
                            commit_hexsha=first_commit.hexsha, unregistered_date=None),
                {"creation_date"})
    _check_dict(rf_l2, dict(object='rf', version='v2', name='production',
                            author=getpass.getuser(),
                            commit_hexsha=second_commit.hexsha, unregistered_date=None),
                {"creation_date"})
    _check_dict(rf_l3, dict(object='rf', version='v2', name='staging',
                            author=getpass.getuser(),
                            commit_hexsha=second_commit.hexsha, unregistered_date=None),
                {"creation_date"})
