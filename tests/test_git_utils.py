...
def test_clone_on_remote_repo_if_repo_gitpython_object_then_leave_it_unchanged(
    ...
    assert_f_called_with_repo_return_repo_itself(repo=Repo(path=tmp_local_empty_git_repo))
...
def test_clone_on_remote_repo_if_repo_is_remote_url_then_clone_and_set_repo_to_its_local_path():
    with patch("gto.git_utils.git_clone") as mocked_git_clone:
        ...
        local_repo = decorated_read_func(repo=tests.resources.SAMPLE_HTTP_REMOTE_REPO, spam=0, jam=3)
        mocked_git_clone.assert_called_once_with(repo=tests.resources.SAMPLE_HTTP_REMOTE_REPO, dir=local_repo)
...
def test_git_push_tag_if_called_with_delete_then_gitpython_corresponding_methods_are_correctly_invoked(
    ...
    git_push_tag(repo_path=path, tag_name=tag_name, delete=True, remote_name=remote_name)
...
def test_git_push_tag_if_error_then_exit_with_code_1(
    ...
    assert "Make sure your local repository is in sync with the remote" in error.value.msg


def test_auto_push_on_remote_repo_if_not_remote_then_auto_push_is_not_changed(
    tmp_local_empty_git_repo,
):
    assert decorated_write_func(spam=37, repo=tmp_local_empty_git_repo, auto_push=True)[0]
    assert not decorated_write_func(spam=37, repo=tmp_local_empty_git_repo, auto_push=False)[0]


@skip_for_windows_py_lt_3_9
def test_auto_push_on_remote_repo_if_remote_then_auto_push_is_set_to_true():
    assert decorated_write_func(spam=37, repo=tests.resources.SAMPLE_HTTP_REMOTE_REPO, auto_push=True)[0]
    assert decorated_write_func(spam=37, repo=tests.resources.SAMPLE_HTTP_REMOTE_REPO, auto_push=False)[0]


def test_auto_push_on_remote_repo_if_not_remote_then_repo_is_not_cloned(
    tmp_local_empty_git_repo,
):
    assert decorated_write_func(spam=37, repo=tmp_local_empty_git_repo, auto_push=True)[1] == tmp_local_empty_git_repo
    assert decorated_write_func(spam=37, repo=tmp_local_empty_git_repo, auto_push=False)[1] == tmp_local_empty_git_repo
...
def test_auto_push_on_remote_repo_if_remote_then_repo_is_cloned(
    ...
    with patch("gto.git_utils.git_clone") as mocked_git_clone:
        mocked_git_clone.side_effect = git_clone
        local_repo = decorated_write_func(repo=tests.resources.SAMPLE_HTTP_REMOTE_REPO, spam=0, auto_push=False)[1]
        mocked_git_clone.assert_called_once_with(repo=tests.resources.SAMPLE_HTTP_REMOTE_REPO, dir=local_repo)
...
def test_git_add_and_commit_all_changes_if_files_not_changed_then_no_new_commit(
    ...
    assert_repo_as_expected_after_first_test_commit(repo_path=tmp_local_git_repo_with_first_test_commit[0])
...
def test_git_commit_specific_files_if_untracked_file_is_changed_then_new_commit(
    ...
    untracked_file = Path(tmp_local_git_repo_with_first_test_commit[0]) / TEST_COMMIT_UNTRACKED_FILE
...
def test_stashed_changes_if_tracked_file_was_changed_then_inside_with_statement_is_rolled_back(
    ...
    tracked_file, _ = change_tracked_file(repo_path=tmp_local_git_repo_with_first_test_commit[0])
...
def test_stashed_changes_if_tracked_file_was_changed_then_outside_with_statement_is_as_before(
    ...
    tracked_file, new_file_content = change_tracked_file(repo_path=tmp_local_git_repo_with_first_test_commit[0])
...
def test_stashed_changes_if_untracked_file_was_changed_but_include_untracked_is_false_then_do_not_roll_back(
    ...
    untracked_file, _ = change_untracked_file(repo_path=tmp_local_git_repo_with_first_test_commit[0])

    with stashed_changes(repo_path=tmp_local_git_repo_with_first_test_commit[0], include_untracked=False):
...
def test_stashed_changes_if_untracked_file_was_changed_then_inside_with_statement_is_rolled_back(
    ...
    with stashed_changes(repo_path=tmp_local_git_repo_with_first_test_commit[0], include_untracked=True):
...
def test_stashed_changes_if_untracked_file_was_changed_then_outside_with_statement_is_as_before(
    ...
    untracked_file, new_file_content = change_untracked_file(repo_path=tmp_local_git_repo_with_first_test_commit[0])
...
def test_commit_produced_changes_on_auto_commit_if_f_changes_tracked_file_already_dangling_then_raise_exception_and_roll_back(
    ...
    with open(tracked_file, "r", encoding="utf") as f:
        assert f.read() == f"{FIRST_TEST_FILE_MODIFICATION} - {SECOND_TEST_FILE_MODIFICATION}"
...
@auto_push_on_remote_repo
def decorated_write_func(spam: int, repo: Union[Repo, str], auto_push: bool):  # pylint: disable=unused-argument
    return auto_push, repo


@clone_on_remote_repo
def decorated_read_func(spam: int, repo: Union[Repo, str], jam: int):  # pylint: disable=unused-argument
    return repo
...
def tmp_local_git_repo_with_first_test_commit(tmp_local_empty_git_repo) -> str:
    ...
    with open(Path(tmp_local_empty_git_repo) / "README.md", "w", encoding="utf") as readme:
...
def mocked_f_decorated_with_commit_produced_changes_on_auto_commit() -> Tuple[
    ...
    @commit_produced_changes_on_auto_commit(message_generator=generate_test_commit_message)
    def f(*args, **kwargs):
    ...
    with patch("gto.git_utils.stashed_changes") as mocked_stashed_changes:
        ...
        with patch(
            ...
            mocked_get_repo_changed_tracked_and_untracked_files.return_value = [TEST_COMMIT_FILE], []
...
def assert_repo_as_expected_after_first_test_commit(repo_path: str) -> None:
    assert list(Repo(path=repo_path).iter_commits())[0].message == FIRST_TEST_COMMIT_MESSAGE
...
def assert_repo_as_expected_after_second_test_commit(repo_path: str, second_commit_on_untracked_file: bool) -> None:
    ...
    if second_commit_on_untracked_file:
        ...
        assert_test_commit_file_content(
            repo_path=repo_path,
            file_name=TEST_COMMIT_FILE,
            expected_content=FIRST_TEST_FILE_MODIFICATION + SECOND_TEST_FILE_MODIFICATION,
        )


def assert_test_commit_file_content(repo_path: str, file_name: str, expected_content: str) -> None:
