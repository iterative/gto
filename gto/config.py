...
def check_requirement_is_valid(name):
...
def assert_requirement_is_valid(name):
    if not check_requirement_is_valid(name):
        raise ValidationError(
            f"Invalid value '{name}'. "
            "Requirements must be in `{name}: {version}` format"
        )
